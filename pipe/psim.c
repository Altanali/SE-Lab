...
void do_fetch_stage()
{
    ...
    imem_error = !get_byte_val(mem, selected_PC, &instruction_byte);
    ...
    switch(HPACK(imem_icode, imem_ifun)) {
        ...
        case HPACK(I_IRMOVQ, F_NONE) : 
            ...
            imem_error |= get_word_val(mem, selected_PC + 2, &(decode_input->valc));
        ...
        case HPACK(I_CALL, F_NONE):
            ...
            imem_error |= get_word_val(mem, selected_PC + 1, &(decode_input->valc));
...
void do_writeback_stage()
{
    ...
    //0s out wb_valM unless the statement is a return statement (for next cycle's select PC)
}
